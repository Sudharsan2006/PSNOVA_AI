...
/* ── App class ───────────────────────────────────────────────────── */
class App {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, autoPlay = false, autoPlayInterval = 3000 } = {}) {
  ...
  startAutoPlay() {
    this.autoPlayTimer = setInterval(() => {
      ...
      // Advance exactly one card forward
      const width = this.medias[0].width;
      const itemIndex = Math.round(Math.abs(this.scroll.target) / width) + 1;
  ...
  onTouchDown(e) {
    ...
    if (this.autoPlay) this.stopAutoPlay(); // pause autoPlay on user interaction
  }
  onTouchMove(e) {
  ...
  onTouchUp() {
    this.isDown = false;
    this.onCheck();
    if (this.autoPlay) this.startAutoPlay(); // resume autoPlay
  }
  onWheel(e) {
...
export default function CircularGallery({
  ...
  useEffect(() => {
    const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoPlay, autoPlayInterval });
    return () => app.destroy();
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoPlay, autoPlayInterval]);
